...
\DeclareGraphicsExtensions{.pdf,.png,.jpg,.svg}
 ...
 \shortTitle{Algorytmy}
	 ...
	 \section{Filtrowanie kolaboratywne}
		...
		W fazie drugiej następuje uczenie (Algorytm  \ref{aq:mf_learn}). Odbywa się ono metodą stochastycznego gradientu prostego. Wynikiem tej fazy są macierze reprezentujące ukryte własności użytkowników i elementów. Mnożąc je ze sobą uzyskiwana jest przewidywana ocena każdego z elementów przez użytkowników. 
		...
		\end{algorithm}
		 ...
		 Algorytm \textit{Biased Matrix Factorization} jest modyfikacją wyżej opisanego algorytmu \textit{Matrix Factorization}. Podobnie jak jego pierwowzór składa się z dwóch faz. W pierwszej fazie dodatkowo inicjowane są dwa dodatkowe wektory: skośność użytkowników (\textit{user bias}) i skośność elementów (\textit{item bias}). 
