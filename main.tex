...
\shortTitle{Ocena eksperymentalna}
	\section{Opis metody badawczej}
		...
		\subsubsection{Yahoo Music}
			...
			YahooMusic to baza zawierająca oceny utworów muzycznych przez użytkowników portalu Yahoo! Music \cite{yahoomusicwebsite}. Baza zawiera 136,736 piosenek i 325,327,926 ocen wystawionych przez 1,823,179 unikalnych użytkowników, średnio 178 ocen na użytkownika i 2,379 ocen na piosenkę. Utwory oceniane są w skali od 1 do 5, gdzie 1 jest oceną najgorszą a 5 najlepszą.
	...
	\section{Przeprowadzone eksperymenty}
		...
		\label{exp:expiterations}
			...
			\begin{longtable}{r||rr|rr|rr}		
				...
				\caption{Zależność błędu algorytmu wyrażonego za pomocą RMSE i czasu wykonania od maksymalnej liczy iteracji.}
			\end{longtable}
		...
		\begin{longtable}{r||rr}
			...
			1        & 1.375336 & 1.071652 \\
			\caption{Zależność błędu algorytmu RMSE od wartości współczynnika bezwładności.}
		\end{longtable}
		...
		\begin{figure}[!ht]
			...
			\caption{Wyniki eksperymentu testującego optymalny rozmiar populacji. Wykres przedstawia zależność pomiędzy rozmiarem a błędem RMSE. 
		...
		Eksperyment miał na celu zbadanie dla jakiej liczby neuronów w warstwie pośredniej algorytm osiąga optymalne rezultaty (zob. \ref{ss:strukturasiecineuronowej}).
		...
		Wyniki eksperymentu przedstawia wykres \ref{fig:exphiddenneural} i tabela \ref{tab:exphiddenneural}. 
		...
		\begin{longtable}{r||rr|rr|rr}
			...
			\caption{Zależność błędu algorytmu RMSE od rozmiaru ukrytej warstwy neuronowej.}
		\end{longtable}
	
		
		\subsubsection{Dopasowanie współczynnika uczenia}
		
			\todo{(NA KOŃCU, BADANIA) Czy trzeba?}
		...
		\todo{To samo dla AmazonMeta}
		
		\subsection{Porównanie zaproponowanych algorytmów hybrydowych z filtrowaniem z analizą zawartości i filtrowaniem kolaboratywnym}
		...
		\subsubsection{Eksperyment pierwszy - efektywność algorytmów w zależności od minimum powtarzających się cech elementów}
		...
		%% END Hybrid movielens2 %%
		
		
		\subsubsection{Badania na bazie AmazonMeta}
		

			\todo{(BADANIA) Porównanie algorytmu hybrydowego z content-based i collaborative}
...
%% END Hybrid amazon2 %%
		...
		\subsection{Analiza statystyczna wyników}		\todo{(PÓŹNIEJ) Analiza statystyczna wyników, PQStat}
