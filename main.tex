...
\DeclareGraphicsExtensions{.pdf,.png,.jpg,.svg}
 ...
 \shortTitle{Algorytmy}
	 ...
	 \section{Filtrowanie kolaboratywne}
		...
		Zadaniem algorytmu jest odnalezienie takiej wartości parametru $w$, dla którego $Q(w)$ będzie minimalne. Wykorzystując klasyczną metodę gradientu prostego poszukiwania minimum można zapisać w następujący sposób:
		...
		gdzie $\eta$ symbolizuje współczynnik uczenia (ang. \textit{learning rate}). W przypadku algorytmu stochastycznego następuje uproszczenie. Gradient $Q(w)$ aproksymowany jest gradientem pojedynczego przypadku:
		...
		Algorytm przetwarza wszystkie elementy zbioru treningowego i dla każdego przypadku wykonuje aktualizację wartości $w$. Przebieg algorytmu przedstawiony jest pseudokodem \ref{aq:sgd}.
